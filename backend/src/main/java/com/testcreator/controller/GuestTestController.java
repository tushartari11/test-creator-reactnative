 ...
 *
 * <p>
 * Handles guest test taking without authentication. Public endpoints - no
...
public class GuestTestController {
        ...
        @Operation(summary = "Generate guest access link", description = "Creates a guest token for unauthenticated test access. Share this link with students.")
        @ApiResponses({
        ...
        public ResponseEntity<GuestTestAccessDTO> generateGuestLink(
                ...
                GuestTestAccessDTO dto = GuestTestAccessDTO.builder()
                                .guestToken(guestToken)
                                .testId(test.getId())
        ...
        @Operation(summary = "Look up test by access code", description = "Finds a published test by its access code and creates a guest session for the student")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Test found, guest session created"),
                        @ApiResponse(responseCode = "404", description = "No published test found with this access code")
        })
        public ResponseEntity<GuestTestAccessDTO> lookupByAccessCode(
                ...
                Test test = testRepository
                                ...
                                .orElseThrow(() -> new ResourceNotFoundException(
                                                "No published test found with this access code"));
        ...
        /**
         * Invalidates (deletes) a guest session that hasn't been used yet.
         * Called when a guest clicks "Different Code" to discard the auto-created session.
         *
         * @param guestToken guest session token to invalidate
         * @return 204 No Content on success
         */
        @DeleteMapping("/sessions/{guestToken}")
        @Operation(summary = "Invalidate guest session", description = "Deletes an unused guest session. Used when a guest wants to try a different access code.")
        @ApiResponses({
        ...
        public ResponseEntity<Void> invalidateGuestSession(
          ...
          GuestTestSession session = guestSessionRepository.findByGuestToken(guestToken)
  ...
  /**
        ...
        @Operation(summary = "Get test details (Guest)", description = "Retrieve test details for anonymous access using guest token")
        @ApiResponses({
        ...
        public ResponseEntity<GuestTestDetailDTO> getTestAsGuest(
                ...
                GuestTestSession session = guestSessionRepository.findValidByGuestTokenWithTest(guestToken)
                                .orElseThrow(() -> new BusinessException("Invalid or expired guest token"));

                Test test = session.getTest();

                GuestTestDetailDTO dto = GuestTestDetailDTO.builder()
                                .guestToken(guestToken)
                                .testId(test.getId())
                                .title(test.getTitle())
        ...
        @Operation(summary = "Get guest attempt", description = "Retrieve guest attempt details (no login required)")
        @ApiResponses({
        ...
        public ResponseEntity<TestAttemptDTO> getGuestAttempt(
                ...
                // Fetch test with questions and options (to avoid LazyInitializationException)
                Test test = testRepository.findByIdWithQuestionsAndOptions(attempt.getTest().getId())
                                .orElseThrow(() -> new ResourceNotFoundException("Test not found"));
        ...
        private TestAttemptDTO mapGuestResultToAttemptDTO(TestAttemptDTO attemptDto,
                        TestResultDTO resultDTO, Test test) {
        ...
        @Operation(summary = "Submit guest answer", description = "Submit answer for a single question during guest test")
        @ApiResponses({
        ...
        public ResponseEntity<Void> submitGuestAnswer(
                ...
                log.info("Guest submitting answer for attempt: {}, question: {}, optionId: {}",
                                attemptId, request.getQuestionId(), request.getSelectedOptionId());

                TestAttempt attempt = testAttemptRepository.findByIdWithTest(attemptId)
                                .orElseThrow(() -> new ResourceNotFoundException("Attempt not found"));
        ...
        @Operation(summary = "Submit guest test", description = "Submit test and immediately see results (no login required)")
        @ApiResponses({
        ...
        private TestAttemptDTO mapGuestAttemptToDTO(TestAttempt attempt, Test test, String guestToken) {
                var questions = test.getQuestions().stream()
                    .sorted(Comparator.comparing(Question::getQuestionNumber))
                                .map(question -> QuestionWithOptionsDTO.builder()
                                                .id(question.getId())
                                                .questionNumber(question.getQuestionNumber())
                                                .questionText(question.getQuestionText())
                                    .options(shuffleOptions(question.getOptions().stream()
                                                                .map(option -> QuestionWithOptionsDTO.OptionDTO
                                                                                .builder()
                                                                                .id(option.getId())
                ...
                long secondsRemaining = expiryTime != null
