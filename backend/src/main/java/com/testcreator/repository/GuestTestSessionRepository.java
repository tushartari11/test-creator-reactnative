...
public interface GuestTestSessionRepository extends JpaRepository<GuestTestSession, Long> {
   ...
   */
  @Query(
      "SELECT gs FROM GuestTestSession gs "
  ...
  /**
   * Finds a valid (not expired, not used) guest session by token with eagerly fetched Test entity.
   *
   * @param guestToken the guest token
   * @return Optional containing the guest session if valid
   */
  ...
  @Query(
      "SELECT gs FROM GuestTestSession gs WHERE gs.guestToken = :guestToken AND gs.expiresAt >"
          + " CURRENT_TIMESTAMP AND gs.isUsed = false")
   ...
   */
  @Query(
      "SELECT CASE WHEN COUNT(gs) > 0 THEN true ELSE false END FROM GuestTestSession gs "
          + "WHERE gs.guestToken = :guestToken "
          + "AND gs.expiresAt > CURRENT_TIMESTAMP "
          + "AND gs.isUsed = false")
