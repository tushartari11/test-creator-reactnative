 ...
 *
 * <p>
 * Provides methods for querying and analyzing violations
 * during test attempts for both students and teachers.
 *
...
public interface ProctoringViolationRepository extends JpaRepository<ProctoringViolation, Long> {
     ...
     */
    @Query("SELECT v FROM ProctoringViolation v WHERE v.attempt.id = :attemptId ORDER BY v.createdAt DESC")
    List<ProctoringViolation> findAllByAttemptIdOrderByCreatedAtDesc(@Param("attemptId") Long attemptId);
    ...
    List<ProctoringViolation> findByAttemptIdAndSeverityIn(
            ...
            @Param("severities") List<ViolationSeverity> severities);
     ...
     */
    @Query("SELECT COUNT(v) > 0 FROM ProctoringViolation v WHERE v.attempt.id = :attemptId AND v.severity = 'CRITICAL'")
