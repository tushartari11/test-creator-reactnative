...
public interface StudentAnswerRepository extends JpaRepository<StudentAnswer, Long> {
   ...
   */
  @Query(
      "SELECT COUNT(sa) FROM StudentAnswer sa "
  ...
  /**
   * Atomically inserts or updates a student answer. Uses PostgreSQL ON CONFLICT to avoid race
   * conditions with concurrent writes.
   *
   * @param attemptId the test attempt ID
   * @param questionId the question ID
  ...
  @Query(
      value =
          "INSERT INTO student_answers (attempt_id, question_id, selected_option, is_correct,"
              + " answered_at) VALUES (:attemptId, :questionId, :selectedOption, :isCorrect,"
              + " :answeredAt) ON CONFLICT (attempt_id, question_id) DO UPDATE SET selected_option"
              + " = :selectedOption, is_correct = :isCorrect, answered_at = :answeredAt",
      nativeQuery = true)
  void upsertAnswer(
