 ...
 *
 * <p>Provides optimized queries for test management including JOIN FETCH to avoid N+1 queries and
...
public interface TestRepository extends JpaRepository<Test, Long> {
  ...
  /**
   * Finds a test by ID with all questions and options loaded (avoids N+1 queries).
   *
  ...
  Page<Test> findByTeacherEmailAndStatus(
      @Param("email") String email, @Param("status") TestStatus status, Pageable pageable);
  ...
  long countByTeacherAndStatus(
      @Param("teacherId") Long teacherId, @Param("status") TestStatus status);
  ...
  @Modifying
  @Query(
      "UPDATE Test t SET t.status = 'ARCHIVED' "
