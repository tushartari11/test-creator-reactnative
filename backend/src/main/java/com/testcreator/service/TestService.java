 ...
 *
 * <p>
 * Handles CRUD operations, validation, and business logic for tests.
 */
@Service
...
public class TestService {
     ...
     */
    public ImportTestResponse importTestFromCsv(MultipartFile file, String testName,
            Integer duration) {
        List<String> errors = new ArrayList<>();
      List<QuestionRequest> questions = CsvQuestionParser.parse(file,
                errors);
        ImportTestResponse response = ImportTestResponse.builder()
                .testName(testName)
    ...
    public Page<TestListDTO> getAllTestsByTeacher(Pageable pageable, String status) {
        ...
        List<TestListDTO> dtos = tests.getContent().stream()
    ...
    private TestDTO mapToTestDTO(Test test, User user) {
        return TestDTO.builder()
                ...
                .accessCode(test.getAccessCode())
                .testDate(test.getTestDate())
                .createdBy(TestDTO.UserInfo.builder()
                        .id(user.getId())
                        .name(user.getName())
                        .email(user.getEmail())
                        .build())
    ...
    private TestDetailDTO mapToTestDetailDTO(Test test) {
        List<TestDetailDTO.QuestionDetailDTO> questionDTOs = test.getQuestions().stream()
                .map(q -> TestDetailDTO.QuestionDetailDTO.builder()
                        .id(q.getId())
                        ...
                        .options(q.getOptions().stream()
                                .map(o -> TestDetailDTO.OptionDTO.builder()
                                        .id(o.getId())
        ...
        return TestDetailDTO.builder()
                ...
                .createdBy(TestDetailDTO.UserInfo.builder()
    ...
    /**
     * Parses a date string in either ISO instant format (with Z) or LocalDateTime
     * format (without Z).
     */
