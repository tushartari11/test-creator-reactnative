 ...
 *
 * <p>All mutating methods scope their work to a single scenario via {@link ScenarioLedger} and run
 * inside a Spring transaction so a thrown exception rolls back the partial inserts atomically.
 */
@Service
@Profile("e2e")
...
public class TestSupportService {
  ...
  public CreateUserResponse createUser(UUID scenarioId, CreateUserRequest req, Role role) {
    ...
    User user =
        User.builder()
            .email(req.email())
            .password(
                passwordEncoder.encode(req.password() == null ? "Password123" : req.password()))
  ...
  public CreateTestResponse createTest(UUID scenarioId, CreateTestRequest req) {
    ...
    User teacher =
        userRepository
            ...
            .orElseThrow(
                () -> new IllegalArgumentException("Teacher not found: " + req.teacherId()));
    if (teacher.getRole() != Role.TEACHER) {
      throw new IllegalArgumentException("User " + req.teacherId() + " is not a TEACHER");
    }

    List<QuestionSeed> seeds =
        req.questions() == null || req.questions().isEmpty()
    ...
    Test test =
        Test.builder()
            .title(
                req.title() == null
                    ? "E2E Test " + scenarioId.toString().substring(0, 8)
                    : req.title())
  ...
  public CreateGuestAccessResponse createGuestAccess(
    ...
    return new CreateGuestAccessResponse(
  ...
  /**
   * Deletes every entity tracked by the scenario, in FK-safe order, atomically. Also purges Redis
   * keys keyed on the scenario's test attempts.
   */
  @Transactional
  public void deleteScenario(UUID scenarioId) {
    ...
    log.debug(
        "Cleaning up scenario {}: users={} tests={} attempts={}",
        scenarioId,
        entities.userIds.size(),
        entities.testIds.size(),
        entities.testAttemptIds.size());
  ...
  private static void validateSeed(QuestionSeed seed, int index) {
    ...
    if (seed.correctOptionNumber() == null
        || seed.correctOptionNumber() < 1
        || seed.correctOptionNumber() > 3) {
      throw new IllegalArgumentException(
