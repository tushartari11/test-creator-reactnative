...
/**
 * Belt-and-braces safety check. Even if the e2e profile is accidentally activated in production,
 * this bean refuses to start unless: 1. app.test-support.enabled = true 2. The datasource URL ends
 * in the configured suffix (default "_e2e")
 *
 * <p>Combined with @Profile("e2e"), this gives three independent guards.
 */
@Component
@Profile("e2e")
...
public class TestSupportStartupGuard {
  ...
  void verifySafeToActivate() {
    ...
    if (suffix == null || suffix.isBlank()) {
      throw new IllegalStateException("app.test-support.allowed-db-url-suffix must be configured.");
    }
    ...
    log.warn(
        "⚠️  Test-support module is ACTIVE. DB={}. Never activate the 'e2e' profile in production.",
        dbName);
  }
