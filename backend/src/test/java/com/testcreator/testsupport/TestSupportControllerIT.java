 ...
 * <ol>
 *   <li>The test-support endpoints are reachable without authentication.</li>
 *   <li>Setup followed by cleanup leaves every relevant table at its baseline row count.</li>
 *   <li>The startup guard requires the DB name to end with {@code _e2e}.</li>
 * </ol>
 */
@Testcontainers
...
class TestSupportControllerIT {
    ...
    void cleanSlate() {
        ...
        jdbc.execute("TRUNCATE proctoring_violations, student_answers, test_attempts, "
    ...
    void healthEndpoint_reportsActive() {
        ResponseEntity<HealthResponse> health = rest.getForEntity(baseUrl() + "/health", HealthResponse.class);
    ...
    void setupThenDelete_leavesDbAtBaseline() {
        ...
        assertThat(afterSetup.get("questions")).isEqualTo(baseline.get("questions") + test.questions().size());
        long expectedOptions = baseline.get("options")
        ...
        assertThat(jdbc.queryForObject("SELECT COUNT(*) FROM users WHERE id IN (?, ?)",
            Long.class, teacherId, studentId)).isZero();
    }
    ...
    private UUID createScenario() {
        ResponseEntity<CreateScenarioResponse> response = rest.postForEntity(
            baseUrl() + "/scenarios", null, CreateScenarioResponse.class);
    ...
    private CreateTestResponse createTest(UUID scenarioId, Long teacherId) {
        ResponseEntity<CreateTestResponse> response = rest.postForEntity(
